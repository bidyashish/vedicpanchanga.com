...
def draw_north_indian_chart(
    ...
    chart: Dict, asc_sign_id: int, lang: str,
) -> None:
    ...
    W = side
    TL = (x0, y0); TR = (x0 + W, y0)
    BR = (x0 + W, y0 + W); BL = (x0, y0 + W)
    T = _midpoint(TL, TR); R = _midpoint(TR, BR)
    B = _midpoint(BR, BL); L = _midpoint(BL, TL)
    C = (x0 + W / 2, y0 + W / 2)

    pdf.set_line_width(0.6)
    pdf.set_draw_color(0, 0, 0)
    pdf.rect(x0, y0, W, W)
    pdf.line(*TL, *BR); pdf.line(*TR, *BL)
    pdf.line(*T, *R); pdf.line(*R, *B); pdf.line(*B, *L); pdf.line(*L, *T)
    ...
    for h in range(1, 13):
        ...
        draw_text(pdf, cx, cy - text_size * 0.3, str(sign_id),
                  LATIN_REGULAR, REGULAR, sign_size, anchor="center")
        ...
        if abbrs:
            ...
            draw_text(pdf, cx, cy + text_size * 0.85, joined,
                      family, BOLD, text_size, anchor="center")
