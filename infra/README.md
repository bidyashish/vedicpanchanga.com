...
| [`setup-monitoring.sh`](setup-monitoring.sh) | Once, optional. Installs Prometheus + Grafana + Node Exporter | **no** - one-shot | yes |
...
6. **Firewall (UFW)** - opens `22/80/443`; blocks `8000/8001` directly.
...
## `setup-monitoring.sh` - optional observability

Installs Prometheus (2.53), Node Exporter (1.8.1), and Grafana. All
services bind to `127.0.0.1` - use SSH tunnels or UFW rules for access.
...
|-- setup-monitoring.sh    Prometheus + Grafana + Node Exporter (one-shot)
+-- README.md              this file
```
