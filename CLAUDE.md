...
- Chandrabalam / Tarabalam live here too (`_chandrabalam` / `_tarabalam`, helpers `chandra_house` / `tara_position`). Good taras are `GOOD_TARA_POSITIONS = {2, 4, 6, 8, 9}` - Janma is **not** good - and good Chandra houses are `{1, 3, 6, 7, 10, 11}`. Both return `segments` (one list per nakshatra / Moon-sign span of the day, with `ends_at`) plus the legacy `good_*` mirror of the first segment. Pinned to DrikPanchang New Delhi 17-19 Sep 2026 in `tests/test_balam.py`; `muhurta._moonsign_at` resolves the Moon sign for the chosen window - never score a window on the sunrise Moon sign.
...
- `festivals.py` - Hindu festival engine behind `GET /api/festivals`: `compute_festivals(year, lat, lon, tz)` scans the year's lunar months (Purnimanta labelling, Adhika detection) and emits tithi / nakshatra / sankranti / eclipse / derived events with local timings plus `periods` (Adhika Masa, Chaturmas, Pitru Paksha, Navratris, Durga Puja). Rule table `FESTIVAL_NAMES_EN` is the id registry; the frontend localizes ids as `fest_<id>` in every locale. Pinned against DrikPanchang New Delhi 2026-2027 in `tests/test_festivals.py` (fixture `tests/fixtures/drikpanchang_festivals_delhi.json`) - do not change expected dates without re-checking Drik. A few rules (Holika Dahan / Raksha Bandhan Bhadra handling, Akshaya Tritiya margin) are fitted empirically to Drik; see the module docstring.
...
- `src/App.tsx` - shell: `TopBar`, **path-routed view switcher** (`/` = panchang, `/kundali`, `/muhurta`, `/transits`, `/festivals`, `/frequency`, `/learn/*`, `/privacy`, `/terms`), shared location state, `Footer`. Old hash URLs (`/#panchang`) are migrated to clean paths on first load. Per-route SEO (title / description / canonical / og:tags) is applied by `lib/seo.applySeo` whenever the view changes. Single-column layout; there is no sidebar or ad rail (Auto ads place themselves).
- `src/pages/` - `KundaliPage.tsx`, `PanchangPage.tsx`, `MuhurtaPage.tsx`, `TransitsPage.tsx`, `FestivalsPage.tsx` (calls `GET /api/festivals` for the shared location + chosen year; periods strip, "Next up", themed hero cards from `lib/festivalThemes.ts`, exact local timings; festival names come from `fest_<id>` locale keys), `FrequencyPage.tsx`, `PrivacyPage.tsx`, `TermsPage.tsx`, plus `articles/` (the seven `/learn/*` pages sharing `ArticleLayout.tsx`). Each page owns its own form state and API calls.
...
- **Clean path routing** (no `#`). Routes: `/` (panchang), `/kundali`, `/muhurta`, `/transits`, `/festivals`, `/frequency`, `/learn/{kundali,planets,panchang,dasha,nakshatras,rashi,vargas}`, `/privacy`, `/terms`. SPA fallback is in nginx (`try_files $uri $uri/ /index.html;`) and Vite dev server does it by default. SSR is not implemented - initial HTML is the same for every URL until React hydrates and `applySeo()` rewrites `<title>` / `<meta>` / canonical link. Sitemap at `/sitemap.xml` lists every clean URL; add new routes there too.
