...
- `src/pages/` - `KundaliPage.tsx`, `PanchangPage.tsx`, `MuhurtaPage.tsx`, `TransitsPage.tsx`, `FestivalsPage.tsx` (static, reads `src/content/festivals/{year}.md` through `lib/festivals.ts` - DrikPanchang New Delhi dates, one 4-column table per month; add a year by adding a file), `FrequencyPage.tsx`, `PrivacyPage.tsx`, `TermsPage.tsx`, plus `articles/` (the seven `/learn/*` pages sharing `ArticleLayout.tsx`). Each page owns its own form state and API calls.
